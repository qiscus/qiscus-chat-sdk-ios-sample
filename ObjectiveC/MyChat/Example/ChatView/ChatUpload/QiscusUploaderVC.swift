...
class QiscusUploaderVC: UIViewController, UIScrollViewDelegate,UITextViewDelegate {
    ...
    func setupUI(){
        ...
        let sendImage = UIImage().image(named: "send")?.withRenderingMode(.alwaysTemplate)
        ...
        self.sendButton.setImage(UIImage().image(named: "ic_send")?.withRenderingMode(.alwaysTemplate), for: .normal)
        
        self.cancelButton.tintColor = ColorConfiguration.sendButtonColor
        self.cancelButton.setImage(UIImage().image(named: "ic_back")?.withRenderingMode(.alwaysTemplate), for: .normal)
    }
