...
class UIChatNavigation2: UIView {
    ...
    func present(room: QChatRoom) {
        ...
        if let room = QiscusCoreManager.qiscusCore2.database.room.find(id: room.id){
        ...
        //load from rest
        if room.type == .group {
            QiscusCoreManager.qiscusCore2.shared.getParticipants(roomUniqueId: (self.room?.uniqueId)!, onSuccess: { (participants, meta) in
