...
class QFileReDownloadLeftCell: UIBaseChatCell {
    @IBOutlet weak var viewReplay: UIView!
    @IBOutlet weak var lbReplyExtras: UILabel!
        ...
        if let messageExtras = message.extras {
            ...
            if !type.isEmpty{
                if type.lowercased() ==  "image_story_reply" {
                    ...
                    self.ivFIle.image = UIImage(named: "ic_ig_gray")?.withRenderingMode(.alwaysTemplate)
                    self.ivFIle.tintColor = ColorConfiguration.leftBaloonTextColor
                }else if (type.lowercased() == "redownload_attachment"){
                    ...
                    self.lbReplyExtras.text = "This attachment file was sent in the previous message by \(sender) on \(dateString), at \(timeString)."
                    
                    self.viewReplay.layer.cornerRadius = 4
                }
            }
        }
    }
    ...
    @IBAction func saveFile(_ sender: Any) {
        if (self.comment?.message.contains("[/file]") == true){
            ...
            if url.contains(".oga") == true {
                ...
                preview.url = url
                preview.roomName = "Document & Audio Preview"
                let backButton = UIBarButtonItem(title: "", style: .plain, target: nil, action: nil)
                backButton.tintColor = UIColor.white
                self.currentViewController()?.navigationItem.backBarButtonItem = backButton
                self.currentViewController()?.navigationController?.pushViewController(preview, animated: true)
