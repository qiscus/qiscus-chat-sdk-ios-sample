...
class QVideoReDownloadLeftCell: UIBaseChatCell {
    @IBOutlet weak var tvContentCons: NSLayoutConstraint!
    ...
    func bindData(message: CommentModel){
        ...
        if let messageExtras = message.extras {
            ...
            if !type.isEmpty{
                if (type.lowercased() == "redownload_attachment"){
                    let sender = dataJson["sender"].string ?? ""
                    let timeStamp = dataJson["timestamp"].int64 ?? 0
                    ...
                    self.lbReply.text = "This attachment file was sent in the previous message by \(sender) on \(dateString), at \(timeString)."
                    
                    self.viewReply.layer.cornerRadius = 4
                    
                    if (tvContent.text?.isEmpty == true) {
                        self.tvContentCons.constant = 0
                    }
                }
            }
        }
