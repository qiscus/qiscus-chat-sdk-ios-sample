...
class QTextReDownloadRightCell: UIBaseChatCell {
    ...
    func bindData(message: CommentModel){
        ...
        if let payload = message.payload {
            if !payload.isEmpty{
                ...
                let sender = json["payload"]["sender"].string ?? ""
                let timestamp = json["payload"]["timestamp"].int64 ?? 0
                ...
                self.tvContent.text = "Redownload Attachment File from \(sender) on \(dateString), at \(timeString)."
            }
        }
