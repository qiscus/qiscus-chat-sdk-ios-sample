        ...
        didSet{
            ...
            if let c = self.comment {
                if c.userEmail == QiscusCore.getProfile()?.email {
                    if cards.count > 0 {
                        if cards.count == 1 {
                            self.carouselLeading.constant = UIScreen.main.bounds.size.width - (UIScreen.main.bounds.size.width * 0.75 + 32)
                        }else{
        ...
        if let c = self.comment {
            ...
            if(self.isPublic == true){
                if c.isMyComment() {
                    ...
                    self.userNameLabel.textColor = ColorConfiguration.leftBaloonColor
                }
